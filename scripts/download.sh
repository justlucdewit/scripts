...
download_version() {
	...
	if [[ -f "$HOME/scripts/versions/$version/build.sh" ]]; then
	...
	wget "https://github.com/justlucdewit/scripts/releases/download/$version/build.sh" -qO "$HOME/scripts/versions/$version/build.sh" >/dev/null
